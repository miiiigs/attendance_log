...
describe("mobile activity loading", () => {
  ...
  it("labels a joined Community activity with the Community name, not Public", async () => {
    ...
    queues["activities"] = [
      makeQuery([]),
      makeQuery([{ id: "act-1", name: "SCPPA Orientation", status: "ended", visibility: "community_only", organization_id: "org-1" }]),
    ];
  ...
  it("labels a public activity (no organization) as Public", async () => {
    ...
    queues["activities"] = [
      makeQuery([]),
      makeQuery([{ id: "act-2", name: "Public Seminar", status: "ended", visibility: "anyone_with_code", organization_id: null }]),
    ];
